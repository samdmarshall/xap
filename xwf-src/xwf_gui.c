 ...
 */
void on_click_column(GtkCList *clist, gint column, gpointer data)
{
	int num;
	GtkCTreeNode *node;
	GList *selection;
...
/*
 */
void cb_open_trash(GtkWidget *item, void *data)
{
	cfg_t *win = (cfg *)data;
 ...
 */
void cb_new_window(GtkWidget *widget, GtkCTree *ctree)
{
	int num;
	GList *selection;
	GtkCTreeNode *node;
 ...
 */
void cb_unselect(GtkWidget *widget, GtkCTree *ctree)
{
						...
						int flags)
{
	...
	while ((de = readdir(dir)) != NULL)
	{
		...
		if (de->d_name[0] == '.')
		{
			...
			{
				if (((flags & IGNORE_HIDDEN) && (de->d_name[1] != '\0')) || (de->d_name[1] == '\0'))
					continue;
			}
		}
 ...
 */
void node_is_open(GtkCTree *ctree, GtkCTreeNode *node, void *data)
{
	GtkCTreeRow *row;
...
/*
 */
int compare_node_path(gconstpointer ptr1, gconstpointer ptr2)
{
 ...
 * empty trash folder
 */
void cb_empty_trash(GtkWidget *widget, GtkCTree *ctree)
{
	GtkCTreeNode *node;
	cfg *win;
	DIR *dir;
 ...
 * open find dialog
 */
void cb_find(GtkWidget *item, GtkWidget *ctree)
{
	GtkCTreeNode *node;
	char cmd[PATH_MAX + 1];
	entry_t *en;
...
/*
 */
void cb_about(GtkWidget *item, GtkWidget *ctree)
{
...
/*
 */
int get_number(GtkCList *ctree, GtkCTreeNode *node)
{
	int num = 0;
	GList *gl;
	gl = ctree->row_list;
 ...
 */
void cb_new_subdir(GtkWidget *item, GtkWidget *ctree)
{
	entry_t *en;
	GtkCTreeNode *node;
 ...
 * new file
 */
void cb_new_file(GtkWidget *item, GtkWidget *ctree)
{
	entry_t *en;
	GtkCTreeNode *node;
	char path[PATH_MAX + 1];
 ...
 */
void cb_new_link(GtkWidget *item, GtkWidget *ctree)
{
	entry_t *en;
	GtkCTreeNode *node;
	char path[PATH_MAX + 1];
 ...
 */
void cb_rename(GtkWidget *item, GtkCTree *ctree)
{
	entry_t *en;
	GtkCTreeNode *node;
	GdkPixmap *pix, *pim;
	...
	if (dlg_string(_("Rename to: "), nfile) == DLG_RC_OK)
	{
		...
		if (strlen(nfile) > 31)
		{
			if (dlg_continue(_("Warning"), _("name has more then 31 characters!")) == DLG_RC_CANCEL)
			{
				return;
			}
		}
 ...
 */
void cb_open_with(GtkWidget *item, GtkCTree *ctree)
{
	entry_t *en;
	cfg_t *win;
	GtkCTreeNode *node;
 ...
 */
void cb_props(GtkWidget *item, GtkCTree *ctree)
{
	entry_t *en;
	GtkCTreeNode *node;
	GList *selection;
 ...
 */
void cb_encrypt(GtkWidget *item, GtkWidget *ctree)
{
	GtkCTreeNode *node;
	int num, i;
	char **files;
...
/*
 */
void on_destroy(GtkWidget *top, cfg_t *win)
{
#ifdef DEBUG
 ...
 */
void cb_destroy(GtkWidget *top, void *data)
{
#ifdef DEBUG
	printf("cb_destroy()\n");
#endif
 ...
 */
void gui_main(char *path, char *xap_path, char *trash, char *reg_file, wgeo_t *geo,
							int flags)
{
	GList *reg;

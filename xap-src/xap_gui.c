 ...
 * gtk initializing
 */
void gui_init(int *argc, char ***argv, char *user_rc) {
  gtk_set_locale();
  gtk_init(argc, argv);
 ...
 */
static void button_clicked(GtkWidget *button, lnk_t *lnk) {
  char *args[3];
  int len, use_shell = 0;
  GdkEvent *event;
 ...
 */
gboolean button_drag_motion(GtkWidget *btn, GdkDragContext *context, gint x,
                            gint y, guint time, void *data) {
  GdkEventCrossing ev;
 ...
 */
void button_drag_leave(GtkWidget *btn, GdkDragContext *cnt, guint time,
                       void *data) {
  GdkEventCrossing ev;
 ...
 */
static void button_drop_data(GtkWidget *button, GdkDragContext *context, gint x,
                             gint y, GtkSelectionData *data, guint info,
                             guint time, lnk_t *lnk) {
  ...
  if ((*lnk->exec != G_DIR_SEPARATOR) || (strchr(lnk->exec, ' ') != NULL)) {
 ...
 */
static void button_drag_data_get(GtkWidget *btn, GdkDragContext *context,
 ...
 * popup button menu
 */
static gint pop_btn_menu(GtkWidget *button, GdkEventButton *event,
                         GtkWidget *menu) {
  if (event->button != 3)
    return (FALSE);
...
/*
 */
static GtkWidget *new_icon(char *file, GdkWindow *window, GdkColor *bg,
                           GdkColormap *cmap) {
...
#endif
  pixmap = gdk_pixmap_colormap_create_from_xpm(NULL, cmap, &mask, bg, file);
  if (pixmap) {
 ...
 */
static void add_program_button(lnk_t *lnk, GtkWidget *box, GtkWidget *menu,
                               GtkTooltips *tips, GdkColormap *cmap) {
  GtkWidget *button, *icon = NULL;

  if (lnk->icon) {
    icon = new_icon(lnk->icon, box->window, &box->style->bg[GTK_STATE_NORMAL],
                    cmap);
  }
  ...
  gtk_drag_dest_set(button, GTK_DEST_DEFAULT_ALL, target_table, NUM_TARGETS,
                    GDK_ACTION_COPY | GDK_ACTION_ASK);
  gtk_drag_source_set(button, GDK_BUTTON1_MASK, target_table, NUM_TARGETS,
 ...
 */
char *find_icon(char *prog) {
  char file[PATH_MAX + 1];
  struct stat st;

  sprintf(file, "%s/%s/.icons/mini-%s.xpm", getenv("XDG_CONFIG_HOME"), XAP_PATH,
          prog);
  if (stat(file, &st) == 0)
    return strdup(file);
 ...
 * globals: Tips
 */
void page_drop_data(GtkWidget *label, GdkDragContext *context, gint x, gint y,
                    GtkSelectionData *data, guint info, guint time,
                    page_t *pg) {
  char *arg, *f, *tf;
    ...
    while (num) {
      ...
      if (uri) {
        ...
        if (!io_is_exec(f)) {
          ...
          lnk_write(path, lnk);
          add_program_button(
              lnk, pg->w_box, pg->w_btn_menu, Tips,
 ...
 */
gboolean page_drag_motion(GtkWidget *label, GdkDragContext *context, gint x,
                          gint y, guint time, int i) {
 ...
 * structure in memory
 */
static lnk_t *entry_or_link(char *file, char *label) {
  lnk_t *lnk = NULL;
  struct stat st;
  char path[PATH_MAX + 1];
  int len;
 ...
 */
void process_dir(GtkWidget *page, char *path, GtkWidget *menu,
                 GtkTooltips *tips, GdkColormap *cmap) {
  DIR *dir;
  struct dirent *de;
  int len;
 ...
 */
page_t *add_page(GtkNotebook *notebook, char *label, char *base, void *btn_mn,
                 int num, int bo) {
  page_t *pg;
  ...
  gtk_drag_dest_set(pg->w_label, GTK_DEST_DEFAULT_ALL, target_table,
 ...
 * globals: Tips
 */
void build_pages(cfg_t *app, char *path, GtkWidget *menu) {
  DIR *dir;
  struct dirent *de;
  ...
  while ((de = readdir(dir)) != NULL) {
    ...
    if (!S_ISDIR(st.st_mode))
      continue;
    app->last++;
    pg = add_page(GTK_NOTEBOOK(app->notebook), de->d_name, path, menu,
                  app->last, app->box_o);
    if (pg)
 ...
 */
static void cb_delete(GtkWidget *item, gpointer *data) {
  lnk_t *lnk;
 ...
 */
static void cb_options(GtkWidget *item, gpointer *data) {
  lnk_t *lnk;
  ...
  if (lnk_opt_change(lnk)) {
    ...
    while (child_list) {
      gtk_container_remove(GTK_CONTAINER(button), GTK_WIDGET(child_list->data));
    ...
    if ((!lnk->icon) || (!icon)) {
      label = gtk_label_new(lnk_get_name(lnk) ? lnk_get_name(lnk) : lnk->exec);
      if (label) {
...
/*
 */
void cb_page_new(GtkWidget *item, gpointer *data) {
  cfg_t *app = (cfg_t *)data;
  char name[DLG_MAX];
  char path[PATH_MAX + 1];
  ...
  if (dlg_string("New page:", name) == DLG_RC_OK) {
    ...
    if (mkdir(path, 0xFFFFFFFF) != -1) {
      app->last++;
      add_page(GTK_NOTEBOOK(app->notebook), name, app->path, app->btn_menu,
               app->last, app->box_o);
    }
  }
}
 ...
 */
void cb_exec(GtkWidget *w, gpointer data) { dlg_execute((char *)data, NULL); }

/*
 * rename a page
 * globals: Tips
 */
void cb_page_rename(GtkWidget *item, gpointer data) {
  cfg_t *app = (cfg_t *)data;
  char name[DLG_MAX], *p;
  char path[PATH_MAX + 1];
  GtkWidget *nth, *label;
  ...
  npg = add_page(GTK_NOTEBOOK(app->notebook), name, app->path, pg->w_btn_menu,
                 current, app->box_o);
  if (npg)
...
/*
 */
void on_page_del(GtkWidget *item, gpointer *data) {}

/*
 * path: default value = "$XDG_CONFIG_HOME/.xap"
 * transient: boolean, should program run as transient window?
 */
void gui_main(char *path, int transient, wgeo_t *geo, int nb_tabpos,
              int nb_boxlayout) {
...
#define LAST_PAGE_MENU (sizeof(page_me) / sizeof(menu_entry))
  ...
  gtk_signal_connect(GTK_OBJECT(top), "destroy", GTK_SIGNAL_FUNC(exit), NULL);

  accel = gtk_accel_group_new();
  _gtk_accel_group_attach(accel, GTK_OBJECT(top));
    ...
    if (button_me[i].func) {
      if (button_me[i].data) {
        gtk_signal_connect(GTK_OBJECT(menu_item), "activate",
                           GTK_SIGNAL_FUNC(button_me[i].func),
                           button_me[i].data);
      } else {
        gtk_signal_connect(GTK_OBJECT(menu_item), "activate",
  ...
  for (i = 0; i < LAST_PAGE_MENU; i++) {
    ...
    if (page_me[i].flags & ACCEL) {
      gtk_widget_add_accelerator(menu_item, "activate", accel, page_me[i].key,
                                 page_me[i].mod, GTK_ACCEL_VISIBLE);
    }
